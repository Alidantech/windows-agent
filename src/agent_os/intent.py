...
class IntentRouter:
    ...
    _DESKTOP_CUES = re.compile(
        ...
        r"rename|move|copy|test|smoke\s+test|inspect|capture|screenshot|close|"
        r"minimi[sz]e|maximi[sz]e|browser|website|web\s*page|page|window|monitor|"
        r"screen|desktop|application|app|chrome|brave|edge|firefox|notepad|calculator|"
        r"explorer|vscode|terminal)\b",
        re.IGNORECASE,
    )
    ...
    def route(
        ...
        *,
        ...
        if self._SCREEN_QUESTION.search(normalized):
            return TaskIntent("desktop", "question requires observing the assigned computer target")
        if browser_active and self._CONTINUATION.search(normalized):
            return TaskIntent("desktop", "continuation refers to the persistent browser session", True)
        aliases = tuple(alias.lower() for alias in app_aliases if alias)
        alias_mentioned = any(re.search(rf"\b{re.escape(alias)}\b", lowered) for alias in aliases)
        if self._URL.search(normalized) or self._DESKTOP_CUES.search(normalized) or alias_mentioned:
            return TaskIntent(
                "desktop",
        ...
        if normalized.endswith("?") or self._QUESTION_START.search(normalized):
            return TaskIntent("conversation", "general question does not require computer control")

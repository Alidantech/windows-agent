            ...
            try:
                ...
                if attempt < self.settings.api_retries:
                    ...
                    if self.cancellation.wait(delay):
                        self.cancellation.raise_if_cancelled()
        raise RuntimeError(
            f"{self.name.title()} request failed after {self.settings.api_retries} attempts: {last_error}"
        ) from last_error

    def list_models(self) -> list[ModelInfo]:
        return [ModelInfo(provider=self.name, model=self.model, details="configured")]
